...
import com.alibaba.datax.my.server.service.impl.ApiServiceImpl;
...
public class JobContainer extends AbstractContainer {
    ...
    /**
     * 源码修改标记
     * 0.方法用来将任务执行的统计信息记录到日志
     * 1.修改：在记录日志前，先使用我们封装的api返回任务状态
     */
    private void logStatistics() {
        ...
        taskStats.setId(this.jobId);
        taskStats.setStartTime(dateFormat.format(startTimeStamp));
        taskStats.setEndTime(dateFormat.format(endTimeStamp));
        taskStats.setTotalCosts(String.valueOf(totalCosts));
        taskStats.setByteSpeedPerSecond(StrUtil.stringify(byteSpeedPerSecond));
        taskStats.setRecordSpeedPerSecond(String.valueOf(recordSpeedPerSecond));
        taskStats.setTotalReadRecords(String.valueOf(CommunicationTool.getTotalReadRecords(communication)));
        taskStats.setTotalErrorRecords(String.valueOf(CommunicationTool.getTotalErrorRecords(communication)));
        taskStats.setIsSuccess(!this.hasException);

        ApiService apiService = new ApiServiceImpl();
        apiService.sendTaskStats(taskStats);
