...
public class Engine {
     ...
     * 源码修改标记
     * 0.该方法为datax执行任务入口
     * 1.entry方法由静态改为了非静态方法
     * 静态的属性和方法程序启动时全部装入内存，永远不会销毁释放。
     * 2.修改方法形参：[from]:final String[] args [to]:DataxTask task
     * 3.修改jobid读取方式：[from]:CommandLine读取 [to]:task传递
     *
     * @param task
     * @throws Throwable
     */
    public void entry(DataxTask task) throws Throwable {
        // jobPath
        String jobPath = task.getJob();
        // 指定jobid - by default = -1
        String jobIdString = task.getId()+"";
        // RUNTIME_MODE = "";
        ...
        // Engine engine = new Engine(); // 源码entry()是静态方法，但在内部（就是这里）却实例化了对象，因为start()非静态
